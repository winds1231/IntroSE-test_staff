...
import { getinsertStaff } from "../../services/getInsertStaffService";
import { editInsertStaff } from "../../services/editInsertStaffService";
import { deleteInsertStaff } from "../../services/deleteInsertStaffService";
...
function InsertStaff() {
  ...
  const fetchData = async () => {
    try {
      const staffData = await getinsertStaff();
  ...
  const handleSaveItem = async (index) => {
    try {
      ...
      // Make a request to edit the staff member using the editInsertStaff function
      await editInsertStaff(
        updatedItem.nv_id,
  ...
  const addNewItem = () => {
    setItems([
      ...items,
      { nv_id: "", hoten: "", gioitinh: "", luong: 0 },
    ]);
  };

  const handleDeleteItem = async (nv_id, index) => {
    try {
      await deleteInsertStaff(nv_id);
      const updatedItems = [...items];
  ...
  return (
    <div className="Insert Staff">
      ...
      <Button cell_button onClick={addNewItem}>Thêm nhân viên</Button>
    </div>
  );
}
